...
class AnovaVMSTransactionStatusAPIView(AuthorizedHotelAPIView):
    process_id = process_constants.ANOVAVMS_TRANSACTION_STATUS

    PARAM_NAMES = AuthorizedHotelAPIView.PARAM_NAMES + ('recordId', 'statusId',)
    PARAM_OVERRIDES = {
        # 'recordId': dict(required_patch=True),
        'statusId': dict(required_patch=True, allowed=(status_constants.TRANSACTION_POSTED,)),
    ...
    def validate(self, request):
        ...
        if self.record.status_id != status_constants.TRANSACTION_QUEUED:
