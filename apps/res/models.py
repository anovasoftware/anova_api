...
class Event(BaseModel):
    ...
    event_start_date    = models.DateField(default=beginning_of_time)
    event_end_date      = models.DateField(default=end_of_time)
...
class EventCategoryPrice(BaseModel):
    ...
    currency                = models.ForeignKey("static.Currency", on_delete=models.CASCADE, related_name='+', default='00')
    rate_type               = models.ForeignKey("static.Type", on_delete=models.CASCADE, related_name='+', default='03B')
    occupancy_type          = models.ForeignKey("static.Type", on_delete=models.CASCADE, related_name='+', default='000')
...
class Guest(BaseModel):
    ...
    reservation               = models.ForeignKey("res.Reservation", on_delete=models.CASCADE, related_name='guests')
    type                      = models.ForeignKey("static.Type", on_delete=models.CASCADE, related_name='+', default='000')
    status                    = models.ForeignKey("static.Status", on_delete=models.CASCADE, related_name='+', default='001')
...
class GuestRoom(BaseModel):
    ...
    type             = models.ForeignKey("static.Type", on_delete=models.CASCADE, related_name='+', default='000')
    status           = models.ForeignKey("static.Status", on_delete=models.CASCADE, related_name='+', default='001')
    arrival_date     = models.DateField(default=end_of_time)
    departure_date   = models.DateField(default=end_of_time)
    static_flag      = models.CharField(max_length=  1, blank=True , unique=False, primary_key=False, default='N')
    internal_comment = models.TextField(blank=True , unique=False, primary_key=False)
    created_date     = models.DateTimeField(auto_now_add=True)
    last_updated     = models.DateTimeField(auto_now=True)
...
class Room(BaseModel):
    ...
    floor               = models.ForeignKey("res.Floor", on_delete=models.CASCADE, related_name='+', default='A000')
    order_by            = models.CharField(max_length=  2, blank=False, unique=False, primary_key=False, default='99')
    code                = models.CharField(max_length= 15, blank=False, unique=False, primary_key=False, default='')
    description         = models.CharField(max_length= 50, blank=False, unique=False, primary_key=False, default='')
    start_date          = models.DateTimeField(default=beginning_of_time)
    end_date            = models.DateTimeField(default=end_of_time)
    effective_status    = models.ForeignKey("static.Status", on_delete=models.CASCADE, related_name="+", default='021')
    lower_berths        = models.IntegerField(default=0)
    upper_berths        = models.IntegerField(default=0)
    maximum_occupancy   = models.IntegerField(default=0)
    accessible_flag     = models.CharField(max_length=  1, blank=False, unique=False, primary_key=False, default='N')
    static_flag         = models.CharField(max_length=  1, blank=True , unique=False, primary_key=False, default='N')
    grouping            = models.CharField(max_length= 30, blank=False, unique=False, primary_key=False, default='')
    internal_comment    = models.TextField(blank=True , unique=False, primary_key=False)
...
class TransactionItem(BaseModel):
    ...
    status                = models.ForeignKey("static.Status", on_delete=models.CASCADE, related_name='+', default='001')
    description           = models.CharField(max_length= 60, blank=True , unique=False, primary_key=False)
    description_extension = models.CharField(max_length= 60, blank=True , unique=False, primary_key=False)
