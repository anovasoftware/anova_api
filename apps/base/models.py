...
class Category(BaseModel):
    ...
    category_key        = models.CharField(max_length= 40, blank=False, unique=False, primary_key=False, default='')
...
# AUTOGEN_BEGIN_ClientPerson#
class ClientPerson(BaseModel):
    client_person_id    = models.CharField(max_length=  6, blank=False, unique=True , primary_key=True )
    client              = models.ForeignKey("static.Client", on_delete=models.CASCADE, related_name='+', default='000')
    ...
    class Meta:
        ...
        ordering            = []
        constraints         = [
            models.UniqueConstraint(fields=['client', 'person'], name='unique_client_person')
        ]
        
    def __str__(self):
        return 'client_person'
# AUTOGEN_END_ClientPerson#
...
class Company(BaseModel):
    ...
    type                = models.ForeignKey("static.Type", on_delete=models.CASCADE, default='000')
    status              = models.ForeignKey("static.Status", on_delete=models.CASCADE, default='001')
    parent_company      = models.ForeignKey("self", on_delete=models.CASCADE, to_field='company_id', related_name="+", default=None, null=True)
    currency            = models.ForeignKey("static.Currency", on_delete=models.CASCADE, related_name="+", default='00')
    code                = models.CharField(max_length= 15, blank=False, unique=False, primary_key=False, default='')
    description         = models.CharField(max_length= 70, blank=False, unique=False, primary_key=False, default='')
    description_long    = models.CharField(max_length=100, blank=False, unique=False, primary_key=False, default='')
    description_short   = models.CharField(max_length= 30, blank=False, unique=False, primary_key=False, default='')
    phone               = models.CharField(max_length= 30, blank=False, unique=False, primary_key=False, default='')
    email               = models.EmailField(blank=True)
    website             = models.URLField(default='', blank=True )
    start_date          = models.DateTimeField(default=beginning_of_time)
    end_date            = models.DateTimeField(default=end_of_time)
    effective_status    = models.ForeignKey("static.Status", on_delete=models.CASCADE, related_name="+", default='021')
    tax_identifier      = models.CharField(max_length= 50, blank=False, unique=False, primary_key=False, default='')
    external_identifier = models.CharField(max_length= 50, blank=False, unique=False, primary_key=False, default='')
